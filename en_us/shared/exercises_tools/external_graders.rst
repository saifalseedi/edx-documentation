...
platform. Course teams build and deploy a grader on an external platform, such
as Amazon Web Services (AWS) or Azure, and then link their external grader to
edX using an interface called `XQueue`_. XQueue is a Representational State
Transfer (RESTful) interface that uses JavaScript Object Notation (JSON)
objects.
...
::
  ...
  curl -v -d "username=&password=" "https://xqueue.stage.edx.org/xqueue/login/"
  curl -v -b "sessionid=returnedbylogin"   "https://xqueue.stage.edx.org/xqueue/get_queuelen/?queue_name=your_q"
...
To create a code response problem in edX Studio, follow these steps.
